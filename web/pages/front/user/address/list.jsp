...
<html>
  ...
  <body>
  	  ...
  	  <table id="table">
  			...
  			<c:forEach items="${addressList}" var="addr" varStatus="addStatus">
 				 	<tr>
 				 		<td>
				...
				<c:if test="${addStatus.count==5}">
					<c:set  var="error" value="${addStatus.count}"/>
				</c:if>
  			</c:forEach>
  		...
  		<form name="f1" action="${pageContext.request.contextPath}/user/order?action=toOrder" method="post">
  ...
  <script type="text/javascript">
  	  ...
  	  function _add(){
		  ...
		  <c:when test="${!empty error}">
		    alert("地址最多只能5个,请删除或修改原有地址！");
		  </c:when>
		  <c:otherwise>
          var url = path+"/pages/front/user/address/add.jsp";
  	  ...
  	  function _chgDft(obj,id){
  		 ...
  		 var url = path+"/user/address?action=changeToDefault&id="+id;
  		 window.location.href=url;
  	  }
  	  //删除一个地址
  	  function _del(id){
  		 var url = path+"/user/address?action=deleteAddress&id="+id;
  		 window.location.href=url;
  	  }
  	  //确定订单
  	  function _toOrder(){
		...
		if(dft){
			...
			consignee=tr.cells[0].innerHTML.trim()+","+tr.cells[1].innerHTML.trim()+","+tr.cells[2].innerHTML.trim();
