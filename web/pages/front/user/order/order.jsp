...
<html>
  ...
  <body>
  	  ...
  	  <div style="background:#F0F0F0;border:1px solid #F0F0FF;">
			<form name="order" action="${pageContext.request.contextPath}/user/order?action=save" method="post">
				<table style="width:100%">
					...
					<tr style="background:#E0E0E0;">
						<td>
  						...
  						<td>
  							<input class="txt" readonly="readonly" type="text" name="sum" id="amt"/>
  					   </td>
  			       </tr>
  			       <tr>
						<td>
  	  ...
  	  <table id="table">
  			...
  			<c:forEach items="${sessionScope.carMap}" var="entry" varStatus="idx">
  				<tr <c:if test="${idx.count%2==0}">class="even"</c:if>>
	  				...
	  				<td>
	  					<fmt:formatNumber value="${entry.value.currentPrice*entry.value.count}" pattern="#.##"/>
	  					<c:set value="${sum+(entry.value.currentPrice*entry.value.count)}" var="sum"/>
